...
root.geometry("+200+0")
root.title("Scientific Fraculator")
clock = Frame(root)
clock.pack(fill=X)
...
label2 = """Input Screen: Always write expression in middle line. Press tab to enter numerator,press tab again to enter denominator then press tab to end fraction."""
...
def createnaviSec():
    ...
    btnup = Button(
        naviSec, text="🠝", command=lambda: disp1.mark_set(INSERT, "insert-1d line")
    )
    btnup.pack(ipady=5)
    btndown = Button(
        naviSec, text="🠟", command=lambda: disp1.mark_set(INSERT, "insert+1d line")
    ...
    btnleft = Button(
        naviSec, text="🠜", command=lambda: disp1.mark_set(INSERT, "insert-1d char")
    )
...
def dispread(display):
    ...
    if "Error" in str(result):
        messagebox.showerror(
            "Expression Error", "Write expression in middle line of input screen.\nRemember: Use * or × for multiplication")
    else:
...
def dispfrac():
    ...
    if disp1.index(INSERT).split(".")[0] == "2":
        ...
        if m > length:
            length = m
        numerator = disp1.get(
            cursor + "-1d line+2d char",
            cursor + "-1d line" + "+" + str(length + 3) + "d char",
        ).strip()
...
Button(
    memSec, text="𝑥 =", font=("Courier", "15", "bold"), command=lambda: send("x")
).grid(row=0, column=0)
...
Button(
    memSec, text="𝑦 =", font=("Courier", "15", "bold"), command=lambda: send("y")
).grid(row=0, column=2)
...
Button(
    memSec, text="𝑧 =", font=("Courier", "15", "bold"), command=lambda: send("z")
).grid(row=0, column=4)
...
disp1 = Text(
    inputscreen, bd=4, font=("Courier", "18", "bold"), height=3, width=40, wrap="none", insertwidth=15
)
disp1.pack()
disp1.focus_set()
disp1.insert(1.0, " " * 40 + "\n" + " " * 40 + "\n" + " " * 40 + "\n")
...
disp2 = Entry(
    outputscreen, bd=2, justify=RIGHT, font=("Courier", "20", "bold"), bg="light grey"
)
